...
const FLOW_POINTS = [
  ...
  "Asesor y recomendación de portafolio.",
];
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-500">
      <div className="flex items-center justify-between">
        <Button
          ...
          onClick={handleBack}
          className="h-9 px-0 text-gray-500 disabled:text-gray-400 disabled:opacity-60"
        >
          <ArrowLeft className="h-4 w-4 mr-1.5" /> Volver
        </Button>
        <Button
          onClick={() => (isLast ? onNext() : setIntroStep(2))}
          className="h-9 px-5 rounded-lg font-semibold gap-2 bg-[#4a7c59] text-white hover:bg-[#3f6b4c] transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0"
        >
      ...
      <div>
        <p className="text-xs font-semibold text-[#6abf1a] uppercase tracking-wider mb-1">Paso 1 · Start up</p>
        <h2 className="text-2xl font-bold text-[#1a1a1a] tracking-tight">Start up</h2>
        <p className="text-sm text-gray-500 mt-1 leading-relaxed">
          A continuación vas a seguir el flujo de onboarding para persona juridica.
        </p>
        <p className="text-sm text-[#4a7c59] mt-2 leading-relaxed font-medium">
          Se viene un proceso de carga de información largo; tranqui, es la parte más difícil. Apenas lo tengamos todo,
          el resto será más fácil y rápido.
        </p>
        <p className="text-xs text-gray-500 mt-1">
          Este mensaje aparece antes de iniciar la ingesta.
        </p>
      </div>

      <Card className="rounded-lg border border-gray-100 shadow-sm gap-0">
        <CardHeader className="px-6 py-5 border-b border-gray-100 space-y-4">
          ...
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 rounded-md bg-[#F0FEE6] flex items-center justify-center shrink-0 animate-in zoom-in-95 duration-300">
              {introStep === 1 ? (
            ...
            <div>
              <CardTitle className="text-lg">
                {introStep === 1 ? "Lo que haremos a continuacion" : "Lo que debes tener a la mano"}
              </CardTitle>
              <CardDescription className="text-sm mt-1">
                {introStep === 1
                  ? "Te guiaremos paso a paso para completar la vinculacion."
                  : "Para que el proceso sea mas facil y rapido."}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
              ...
              <div className="space-y-2">
                {DOC_CATEGORIES.map((category, idx) => {
                  ...
                  return (
                    <details
                      key={category.title}
                      className="group rounded-lg border border-gray-100 bg-white open:bg-[#fcfef8]"
                      open={isOpen}
                      onClick={(e) => {
                        e.preventDefault();
                        setActiveCategory(idx);
